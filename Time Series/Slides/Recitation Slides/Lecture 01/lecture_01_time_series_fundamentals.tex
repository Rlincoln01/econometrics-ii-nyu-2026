...
% ----------------------------------------------------------------------------
\begin{frame}{Example: Prediction from a Time Series}
    \begin{center}
        \includegraphics[width=0.92\textwidth]{figures/prediction_ex.png}
    \end{center}
\end{frame}
...
\begin{frame}{Example: Estimating a Phillips Curve}
    ...
    \begin{itemize}
        ...
        \item Is $\beta_2 \approx 1$? (Testing \textit{adaptive expectations}: $\E_{t-1}[\pi_t] = \pi_{t-1}$)
    \end{itemize}
...
\begin{frame}{Stylized Facts of Time Series}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Cycles}: Cyclical variation with period $>$ 1 year (business cycles)
...
\begin{frame}[plain]
    \begin{center}
        {\Large \textbf{Part II: Stochastic Processes}}\\[1em]
        {\large Section 7.2.1: Formal Definitions}
    \end{center}
\end{frame}
...
\begin{frame}[plain]
    \begin{center}
        {\Large \textbf{Part II: Stochastic Processes}}\\[1em]
        {\large Section 7.2.2: Models for Stochastic Processes}
    \end{center}
\end{frame}
...
\begin{frame}{Data-Generating Process (DGP)}
    \begin{definition}[Data-Generating Process]
        The \textbf{DGP} is the probability law describing the stochastic process $\{X_t\}_{t \geq 0}$ conditional on available information:
        \[
        F_t(\{X_t\}_{t \geq 0} | \F_{t-1}^X)
        \]
        --- the true joint distribution of the process.
    \end{definition}
    
    \vspace{1em}
    
    \textbf{Problem}: This is an \textit{infinite-dimensional} distribution!
    \begin{itemize}
...
\begin{frame}{Correct Specification Axioms}
    \begin{block}{Axiom: Correct Specification for Distribution}
        \[
        \mathcal{M}_D(\xbf_t, \xbf_{t-1}, \ldots, d_t; \psi) = F_t(\{X_t\}_{t \geq 0} | \F_{t-1}^X)
        \]
    \end{block}
...
\begin{frame}{Strong Stationarity}
    \begin{definition}[Strong Stationarity]
        A stochastic process $\{Y_t\}$ is \textbf{strongly stationary} (strictly stationary) if the joint distribution is invariant to time shifts:
        \[
        F(y_1, \ldots, y_T) = F(y_{1+h}, \ldots, y_{T+h}) \quad \forall h = \pm 1, \pm 2, \ldots
        \]
    \end{definition}
...
\begin{frame}[plain]
    \begin{center}
        {\Large \textbf{Part II: Stochastic Processes}}\\[1em]
        {\large Section 7.2.3: Linear Operators}
    \end{center}
\end{frame}
...
\begin{frame}[plain]
    \begin{center}
        ...
        {\large Section 7.3: Applications to Macroeconomics}
    \end{center}
\end{frame}

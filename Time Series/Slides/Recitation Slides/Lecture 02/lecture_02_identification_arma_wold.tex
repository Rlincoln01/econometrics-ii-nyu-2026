...
\subtitle{Recitation Session 2: Reduced-Form Time Series Analysis}
...
\begin{frame}{Today's Roadmap}
    ...
    \vspace{0.5em}
    \textbf{Part 2: Exercises}
    \begin{itemize}
        \item Pset 1 (Problems 1--8)
    \end{itemize}
\end{frame}
...
\begin{frame}{Causality and Invertibility: General Definitions}
    Let $\{Y_t\}$ be a stationary process and $\{Z_t\}$ an input/shock process.
    ...
    \begin{itemize}
        \item $Y_t$ depends only on \textbf{current and past} values of $Z$ --- no future inputs needed.
        \item The shocks $Z_t$ can be \textbf{recovered from current and past observables} $Y_t$.
    \end{itemize}
...
% ----------------------------------------------------------------------------
\begin{frame}{VARMA: Causality and Invertibility}
    \begin{definition}
        A VARMA process $\{y_t\}$ is \textbf{causal} w.r.t.\ $\{z_t\}$ if $y_t \in \Span(z_\tau, -\infty < \tau \leq t)$
        
        \vspace{0.2em}
        
        It is \textbf{invertible} w.r.t.\ $\{z_t\}$ if $z_t \in \Span(y_\tau, -\infty < \tau \leq t)$
    \end{definition}
    
    \textbf{Sufficient conditions} for $\Phi(L) y_t = \Theta(L) z_t$:
    \begin{itemize}
        \item \textbf{Causality}: $\Phi(L)$ has 1-sided inverse $\;\Rightarrow\;$ VMA($\infty$): $y_t = \Psi(L) z_t$
        
        \item \textbf{Invertibility}: $\Theta(L)$ has 1-sided inverse $\;\Rightarrow\;$ VAR($\infty$): $\Pi(L) y_t = z_t$
    \end{itemize}
    ...
    \textbf{Key takeaways}:
    \begin{itemize}
        \item Structural macro models yield VMA representations $\Rightarrow$ always causal
        \item Invertibility is \textbf{far from guaranteed}! (e.g., 5 shocks but 2 observables)
    \end{itemize}
\end{frame}
...
% ----------------------------------------------------------------------------
\begin{frame}{MA($q$): Invertibility}\label{slide:invertibility_definition}
    \begin{definition}[Invertibility]
    ...
    \textbf{Practical importance}: Invertibility allows expressing $u_t$ as function of observables --- essential for estimation! \hfill \hyperlink{slide:pset1_prob1_solution}{\beamerbutton{Example: Pset 1, Problem 1}}
...
\begin{frame}{Wold's Representation Theorem}
    \begin{theorem}[Wold, 1938]
        ...
        \begin{enumerate}
            ...
            \item $u_t = y_t - \E^*[y_t \mid\mathcal{F}^Y_{t-1}]$ (one-step-ahead forecast errors)
...
% ----------------------------------------------------------------------------
% EXERCISES (Pset 1) -- Ordered
% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 1: Nonstationary bivariate system}
    % \footnotesize
    \begin{block}{Problem}
        ...
        \begin{enumerate}[(a)]
            \item Show that the vector of first differences $[\Delta y_{1t},\Delta y_{2t}]'$ is a \textbf{non-invertible} MA(1) process.
            \item Are there observationally equivalent, \textbf{invertible} representations? Why or why not?
        \end{enumerate}
    \end{block}
\end{frame}

\begin{frame}{Pset 1 --- Problem 1: Solution (VMA(1) form)}\label{slide:pset1_prob1_solution}
    \footnotesize
    First differences:
    \[
    \Delta y_{1t}=\varepsilon_{1t}+\theta\varepsilon_{1,t-1},\qquad
    \Delta y_{2t}=\phi\Delta y_{1t}+(\varepsilon_{2t}-\varepsilon_{2,t-1}).
    \]
    Define $u_t=(\varepsilon_{1t},\varepsilon_{2t})'$ and rearrange the system as
    \[
    \underbrace{\begin{pmatrix}1&0\\ -\phi&1\end{pmatrix}}_{\Omega}\Delta y_t
    =
    \underbrace{\begin{pmatrix}1&0\\ 0&1\end{pmatrix}}_{\Pi_0}u_t
    +
    \underbrace{\begin{pmatrix}\theta&0\\ 0&-1\end{pmatrix}}_{\Pi_1}u_{t-1}.
    \]
    Premultiplying by $\Omega^{-1}=\begin{pmatrix}1&0\\ \phi&1\end{pmatrix}$ gives the VMA(1) form:
    \[
    \Delta y_t
    =
    \underbrace{\Omega^{-1}\Pi_0}_{A_0=\begin{pmatrix}1&0\\ \phi&1\end{pmatrix}}u_t
    +
    \underbrace{\Omega^{-1}\Pi_1}_{A_1=\begin{pmatrix}\theta&0\\ \phi\theta&-1\end{pmatrix}}u_{t-1}
    \;=\;
    (A_0+A_1L)u_t,
    \]
    so it is a VMA(1).
...
\begin{frame}{Pset 1 --- Problem 1: Solution (Non-invertibility)}
    \footnotesize
    \hfill \hyperlink{slide:invertibility_definition}{\beamerbutton{Back: Invertibility}}
    
    \vspace{0.3em}
    \textbf{Non-invertibility}: check the determinant
    \[
    \det(A_0+A_1z)=\det\!\begin{pmatrix}1+\theta z&0\\ \phi(1+\theta z)&1-z\end{pmatrix}
    =(1+\theta z)(1-z),
    \]
    which has a root at $z=1$ (on the unit circle) $\Rightarrow$ fails invertibility.
    
    \vspace{0.3em}
    \textbf{(b)} No observationally equivalent \emph{invertible} MA(1) representation within the usual class: the problematic root is exactly at $z=1$ and cannot be ``flipped'' outside the unit circle.
    
    \vspace{0.3em}
    \textbf{Contrast with MA root-flipping}: in the MA(1) example earlier, a non-invertible root \emph{inside} the unit circle can be flipped outside by mapping $\theta \leftrightarrow 1/\theta$ (and rescaling the innovation variance), yielding an observationally equivalent \emph{invertible} representation. Here the offending root is exactly at $z=1$, and $1/z = 1$ --- so there is \textbf{no distinct flipped representation} to restore invertibility.
    \begin{block}{Key takeaway}
        Non-invertibility can be \textbf{structural} (unit-circle root), not just a choice of MA parameterization.
    \end{block}
\end{frame}
...
% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 2: Measurement error in MA(1)}
    \begin{block}{Problem}
...
\begin{frame}{Pset 1 --- Problem 2: Solution (a)}
    ...
    \begin{align*}
    \gamma_0 &= \Var(x_t) = \Var(y_t) + \Var(u_t) \\
    & =  \Var(\varepsilon_t) + \theta^2 \Var(\varepsilon_{t-1}) + \Var(u_t) \\
    &= \sigma_\varepsilon^2 + \theta^2 \sigma_\varepsilon^2 + \sigma_u^2 = \boxed{(1 + \theta^2)\sigma_\varepsilon^2 + \sigma_u^2}
    \end{align*}
\end{frame}

\begin{frame}{Pset 1 --- Problem 2: Solution (b)}
...
\begin{frame}{Pset 1 --- Problem 2: Solution (c)}
...
% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 3: Finite MA($q$) properties}
    % \footnotesize
    \begin{block}{Problem}
        A finite-order MA($q$) process can be expressed as
        \[
        y_t = \sum_{j=0}^{q}\theta_j \varepsilon_{t-j}, \qquad \varepsilon_t \text{ is white noise, } \theta_0 = 1.
        \]
        Show that all finite-order moving-average processes:
        \begin{enumerate}[(a)]
            \item are covariance stationary;
            \item have autocovariances that truncate at lag $q$ (i.e., $\gamma_k \neq 0$ for $k\le q$ and $\gamma_k=0$ for $k>q$);
            \item have absolutely summable autocovariances/autocorrelations.
        \end{enumerate}
    \end{block}
    
    \vspace{0.3em}
    \textbf{Idea}: expand products using WN orthogonality ($\E[\varepsilon_t\varepsilon_s]=0$ for $t\neq s$).
\end{frame}

\begin{frame}{Pset 1 --- Problem 3: Solution}
    \footnotesize
    Let $\E[\varepsilon_t]=0$, $\Var(\varepsilon_t)=\sigma_\varepsilon^2$, and $\E[\varepsilon_t\varepsilon_s]=0$ for $t\neq s$.
    
    \vspace{0.3em}
    \begin{enumerate}[(a)]
        \item \textbf{Covariance stationarity}: $\E[y_t]=\sum_{j=0}^q \theta_j \E[\varepsilon_{t-j}]=0$ (constant).
        
        Variance:
        \[
        \gamma_0=\Var(y_t)=\sigma_\varepsilon^2\sum_{j=0}^q \theta_j^2 < \infty .
        \]
        \item \textbf{Autocovariance truncation}: for $k\ge 0$,
        \[
        \gamma_k=\Cov(y_t,y_{t-k})
        =\sigma_\varepsilon^2\sum_{j=0}^{q-k}\theta_{j+k}\theta_j,
        \]
        because only terms with matching innovation indices survive. If $k>q$, the sum is empty $\Rightarrow \gamma_k=0$.
        \item \textbf{Absolute summability}: since $\gamma_k=0$ for $|k|>q$,
        \[
        \sum_{k=-\infty}^{\infty}|\gamma_k|=\sum_{k=-q}^{q}|\gamma_k|<\infty
        \quad\Rightarrow\quad
        \sum_{k=-\infty}^{\infty}|\rho_k|<\infty.
        \]
    \end{enumerate}
    % \begin{block}{Key takeaway}
    %     Finite MA $\Rightarrow$ always covariance stationary and has \textbf{short memory}: ACF cuts off after $q$.
    % \end{block}
\end{frame}

% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 4: Sum of two AR(1) processes}
    \begin{block}{Problem}
...
\begin{frame}{Pset 1 --- Problem 4: Solution (a)}
...
\begin{frame}{Pset 1 --- Problem 4: Solution (b) Part 1}
    \textbf{(b) Case $\phi_1 \neq \phi_2$}:
...
\begin{frame}{Pset 1 --- Problem 4: Solution (b) Part 2}
    \textbf{Expanding}:
    \[
...
% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 5: MA(2) invertibility \& observational equivalence}
    % \footnotesize
    \begin{block}{Problem}
        Consider a second-order moving-average process
        \[
        x_t=\varepsilon_t+\theta_1\varepsilon_{t-1}+\theta_2\varepsilon_{t-2},
        \qquad \varepsilon_t \overset{iid}{\sim}N(0,\sigma^2).
        \]
        \begin{enumerate}[(a)]
            \item State conditions on $(\theta_1,\theta_2)$ for the process to be \textbf{invertible}.
            \item Characterize the family of \textbf{observationally equivalent}, non-invertible MA(2) representations.
        \end{enumerate}
    \end{block}
\end{frame}

\begin{frame}{Pset 1 --- Problem 5: Solution (a) Invertibility}
    \footnotesize
    Write $x_t=\Theta(L)\varepsilon_t$ with $\Theta(L)=1+\theta_1L+\theta_2L^2$.
    
    \vspace{0.3em}
    \textbf{Invertibility} $\iff$ all roots of $\Theta(z)=0$ satisfy $|z|>1$.
    
    \vspace{0.3em}
    Equivalently, factor
    \[
    \Theta(L)=(1-\lambda_1L)(1-\lambda_2L),
    \qquad
    \lambda_1+\lambda_2=-\theta_1,\;\;\lambda_1\lambda_2=\theta_2.
    \]
    Then invertibility $\iff |\lambda_1|<1$ and $|\lambda_2|<1$ (roots outside the unit circle).
\end{frame}

\begin{frame}{Pset 1 --- Problem 5: Solution (b) Observational equivalence + roots}
    \footnotesize
    \textbf{OE comes from second moments}: $\Gamma_x(h)=\Gamma_{\tilde x}(h)\;\forall h$
    $\iff$ $\Omega_x(z)=\Omega_{\tilde x}(z)$.
    
    \vspace{0.2em}
    \textbf{MA($q$)}: if $x_t=\Theta(L)\varepsilon_t$ with $\Var(\varepsilon_t)=\sigma^2$, then
    \[
    \Omega_x(z)=\Theta(z)\Theta(z^{-1})\,\sigma^2
    \quad\Rightarrow\quad
    \Theta(z)\Theta(z^{-1})\,\sigma^2=\widetilde\Theta(z)\widetilde\Theta(z^{-1})\,\tilde\sigma^2.
    \]
    
    \vspace{0.3em}
    \textbf{MA(2) case-by-case} (write $\Theta(L)=(1-\lambda_1L)(1-\lambda_2L)$):
    \begin{itemize}
        \item \textbf{No flip}: $\widetilde\Theta(L)=(1-\lambda_1L)(1-\lambda_2L)$,\;\; $\tilde\sigma^2=\sigma^2$.
        \item \textbf{Flip root 1}: $\widetilde\Theta(L)=(1-\lambda_1^{-1}L)(1-\lambda_2L)$,\;\; $\tilde\sigma^2=\lambda_1^{2}\sigma^2$.
        \item \textbf{Flip root 2}: $\widetilde\Theta(L)=(1-\lambda_1L)(1-\lambda_2^{-1}L)$,\;\; $\tilde\sigma^2=\lambda_2^{2}\sigma^2$.
        \item \textbf{Flip both}: $\widetilde\Theta(L)=(1-\lambda_1^{-1}L)(1-\lambda_2^{-1}L)$,\;\; $\tilde\sigma^2=(\lambda_1\lambda_2)^{2}\sigma^2=\theta_2^2\sigma^2$.
    \end{itemize}
\end{frame}

% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 6: Sum of independent AR(1) processes}
    \begin{block}{Problem}
...
\begin{frame}{Pset 1 --- Problem 6: Solution (a)}
...
\begin{frame}{Pset 1 --- Problem 6: Solution (b) Part 1 --- Innovation}
    ...
    \textbf{Define the innovation $\eta_t$}: The right-hand side is
    \[
    \eta_t \;\equiv\; (1-\rho_2 L)u_t + (1-\rho_1 L)v_t
    \;=\; u_t+v_t-\rho_2 u_{t-1}-\rho_1 v_{t-1}.
    \]
    Since $\eta_t$ is MA(1), it admits the representation $\eta_t=(1+\theta L)\varepsilon_t$ for some white noise $\varepsilon_t$.
\end{frame}

\begin{frame}{Pset 1 --- Problem 6: Solution (b) Part 2 --- Autocovariances}
    \textbf{Identification via autocovariances}: For $\eta_t = (1+\theta L)\varepsilon_t$:
    \[
    ...
    From the expression $\eta_t = u_t+v_t-\rho_2 u_{t-1}-\rho_1 v_{t-1}$:
    \[
...
\begin{frame}{Pset 1 --- Problem 6: Solution (b) Part 3}
    \textbf{Solving for $\theta$}: The lag-1 autocorrelation of an MA(1) is:
    \[
    ...
    {\small
    \begin{itemize}
        \item Matching autocorrelations uses $\rho_1=\gamma_1/\gamma_0$, so the unknown scale $\sigma_\varepsilon^2$ cancels out.
        \item It gives one equation in one unknown ($\theta$), hence a (generally two-root) identification of the MA(1) parameter.
    \end{itemize}
    }
    
    This implies a quadratic equation in $\theta$.
...
\begin{frame}{Pset 1 --- Problem 6: Solution (c)}
...
\begin{frame}{Pset 1 --- Problem 6: Solution (d)}
    \textbf{(d) Showing that $\theta \neq \pm 1$}: Suppose $\theta = 1$. Then $\rho_1^{\text{MA}} = \frac{1}{1+1} = \frac{1}{2}$. For this to occur, we would need:
    \[
    ...
    Key inequality: for any real $x$, $2|x|\le 1+x^2$, with strict inequality when $|x|<1$.
    Since $|\rho_1|,|\rho_2|<1$ and $\sigma_u^2,\sigma_v^2>0$,
    \[
    2\big(|\rho_2|\sigma_u^2+|\rho_1|\sigma_v^2\big)
    < (1+\rho_2^2)\sigma_u^2+(1+\rho_1^2)\sigma_v^2.
    \]
    But $|\rho_2\sigma_u^2+\rho_1\sigma_v^2|\le |\rho_2|\sigma_u^2+|\rho_1|\sigma_v^2$, hence
    \[
    \left|\frac{-\rho_2\sigma_u^2-\rho_1\sigma_v^2}{(1+\rho_2^2)\sigma_u^2 + (1+\rho_1^2)\sigma_v^2}\right|
    < \frac{1}{2}.
    \]
...
% \begin{frame}{Pset 1 --- Problem 6: Solution (e)}
%     \textbf{(e) Forecasting using the ARMA(2,1)}: From the model $[1 - (\rho_1+\rho_2)L + \rho_1\rho_2 L^2]z_t = (1+\theta L)\varepsilon_t$:
%     \[
...
% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 7: Partial adjustment vs.\ adaptive expectations}
    \begin{block}{Partial Adjustment Model}
        The desired level $y_t^*$ is related to the level of $x_t$ by: $y_t^* = \beta x_t$. $x_t$ is covariance-stationary. However, only a fraction $\gamma$ of the disequilibrium between the desired level and the actual level is removed through partial adjustment:
        \[
...
\begin{frame}{Pset 1 --- Problem 7: Solution (a)}
    ...
    \implies y_t &= \gamma\beta x_t + (1-\gamma)y_{t-1} + e_t
    \end{align*}
    ...
    which we can rewrite as: $y_t = \beta\lambda x_t + (1-\lambda)y_{t-1} + v_t - (1-\lambda)v_{t-1}$
\end{frame}

\begin{frame}{Pset 1 --- Problem 7: Solution (a) Cont.}
...
\begin{frame}{Pset 1 --- Problem 7: Solution (b)}
    ...
    \begin{itemize}
        ...
        \item $\Cov(x_t, e_t) = 0$ (strict exogeneity)
    ...
    \begin{itemize}
        ...
        \item $\Cov(y_{t-1}, \eta_t) = \Cov(y_{t-1}, v_t -(1-\lambda)v_{t-1}) \neq 0$
...
\begin{frame}{Pset 1 --- Problem 7: Solution (c)}
    ...
    \begin{alertblock}{Caution}
        If the correct model is adaptive expectations, use appropriate estimation methods (such as GMM, IV, etc.) instead of OLS.
    \end{alertblock}
\end{frame}
...
% ----------------------------------------------------------------------------
\begin{frame}{Pset 1 --- Problem 8: Vector model (inflation, output gap, policy)}
    
    \begin{block}{Macroeconomic Model}
        Consider the 3 equation new-keynesian model of monetary policy transmission:
        \begin{align*}
...
\begin{frame}{Pset 1 --- Problem 8: Tasks (cont.)}
    \textbf{Tasks}:
    \begin{enumerate}[(a)]
        \item Show that $\pi_t$ follows an ARMA$(p,q)$. Relate the coefficients to the structural parameters.
        
        \item Consider $\rho = 0, 1, 2$. What can be said about inflation and monetary policy? 
        % Simulate 200 observations in MATLAB.
...
\begin{frame}{Pset 1 --- Problem 8: Solution (a) Part 1}
    \textbf{(a) Deriving the ARMA process for $\pi_t$}:
    
    \vspace{0.3em}
    
    \textbf{Step 1}: Use (3) to replace $\pi_t^e$ in (2), and use (4) (lagged) to replace $i_{t-1}$ in (2):
    \begin{align*}
...
\begin{frame}{Pset 1 --- Problem 8: Solution (a) Part 2}
...
\begin{frame}{Pset 1 --- Problem 8: Solution (b)}
...
\begin{frame}{Pset 1 --- Problem 8: Solution (c)}
...
\begin{frame}{Pset 1 --- Problem 8: Solution (d)}
...
\begin{frame}{Pset 1 --- Problem 8: Solution (e)}
    ...
    \textbf{Effect on the model}: inflation now depends on the entire past history, not only on $\pi_{t-1}$. Rewrite the above equation as:
    \[
...
\begin{frame}{Exercise Session: Summary}
    ...
    \begin{alertblock}{For Next Session}
        Review the exercises we did not do in class due to time constraints
    \end{alertblock}
\end{frame}

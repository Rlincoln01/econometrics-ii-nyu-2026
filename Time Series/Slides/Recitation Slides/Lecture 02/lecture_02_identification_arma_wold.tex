...
\begin{frame}{Today's Roadmap}
    ...
    \begin{itemize}
        \item Pset 1 (Problems 1--8): Tim + Medeiros practice problems
    \end{itemize}
\end{frame}
...
\begin{frame}{Pset 1 --- Problem 5: Solution}
    \footnotesize
    Write $x_t=\Theta(L)\varepsilon_t$ with $\Theta(L)=1+\theta_1L+\theta_2L^2$.
    
    \vspace{0.3em}
    \begin{enumerate}[(a)]
        \item \textbf{Invertibility} $\iff$ all roots of $\Theta(z)=0$ satisfy $|z|>1$.
        
        Equivalently, factor $\Theta(L)=(1-\lambda_1L)(1-\lambda_2L)$ with
        $\lambda_1+\lambda_2=-\theta_1$, $\lambda_1\lambda_2=\theta_2$.
        Then invertibility $\iff |\lambda_1|<1$ and $|\lambda_2|<1$ (roots outside unit circle).
        
        \item \textbf{Observationally equivalent non-invertible family (root flipping)}:
        
        If $\Theta(L)=\prod_{i=1}^2(1-\lambda_iL)$, pick any subset $S\subseteq\{1,2\}$ and define
        \[
        \widetilde\Theta(L)=\prod_{i\in S}(1-\lambda_i^{-1}L)\prod_{i\notin S}(1-\lambda_iL),
        \]
        i.e., \emph{replace} any factor with $|\lambda_i|<1$ by its reciprocal (moving the corresponding root \emph{inside} the unit circle).
        
        With suitably rescaled white noise $\tilde\varepsilon_t$ (variance scaled by $\prod_{i\in S}\lambda_i^2$), the process
        $x_t=\widetilde\Theta(L)\tilde\varepsilon_t$ has the \textbf{same second moments} as the original MA(2), hence is observationally equivalent.
    \end{enumerate}
    \begin{block}{Key takeaway}
        For MA models, \textbf{second moments cannot distinguish} between invertible and root-flipped non-invertible representations.
    \end{block}
\end{frame}
...
\begin{frame}{Pset 1 --- Problem 1: Solution}
    \footnotesize
    First differences:
    \[
    ...
    \]
    Define $u_t=(\varepsilon_{1t},\varepsilon_{2t})'$ and write
    \[
    \Delta y_t
    =
    \underbrace{\begin{pmatrix}1&0\\ \phi&1\end{pmatrix}}_{A_0}u_t
    +
    \underbrace{\begin{pmatrix}\theta&0\\ \phi\theta&-1\end{pmatrix}}_{A_1}u_{t-1}
    \;=\;
    (A_0+A_1L)u_t,
    \]
    so it is a VMA(1).
...
\begin{frame}{Pset 1 --- Problem 6: Solution (b) Part 1}
...
\begin{frame}{Pset 1 --- Problem 6: Solution (b) Part 2}

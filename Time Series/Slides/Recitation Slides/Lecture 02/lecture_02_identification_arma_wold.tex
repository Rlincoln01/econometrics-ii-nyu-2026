...
\begin{frame}{Causality in Time Series}
    Following Wolf (2022), we define causality for VARMA processes:
    
    \begin{definition}[Causal Process]
    ...
    \textbf{Interpretation}: The current value $y_t$ can be expressed as a function of \textbf{current and past} values of $z_t$ only --- no future values needed.
    
    \vspace{0.5em}
    
    \begin{block}{Sufficient Condition}
        For VMA: $y_t = \Theta(L)z_t$, causality holds if $\Theta(L)$ is a \textbf{one-sided} polynomial in the lag operator (only non-negative powers of $L$).
        
        \vspace{0.2em}
        
        This gives the VMA($\infty$) representation we use throughout.
    \end{block}
\end{frame}

% ----------------------------------------------------------------------------
\begin{frame}{Invertibility in Time Series}
    \begin{definition}[Invertibility]
        A VARMA process $\{y_t\}$ is \textbf{invertible} with respect to $\{z_t\}$ if:
        \[
    ...
    \textbf{Interpretation}: We can recover the innovations $z_t$ from \textbf{current and past observables} $y_t$.
    
    \vspace{0.5em}
    
    \begin{block}{Sufficient Condition}
        For VMA: $y_t = \Theta(L)z_t$, invertibility holds if $\Theta(L)$ has a \textbf{one-sided inverse}, giving a VAR($\infty$) representation:
        \[
        \Theta(L)^{-1}y_t = z_t
        \]
    \end{block}
    ...
    \begin{alertblock}{Warning (Wolf, 2022)}
        Invertibility is \textbf{far from guaranteed} in structural models!
        
        E.g., if we have 5 shocks but only 2 observables $\Rightarrow$ cannot possibly be invertible.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Part I: Summary}
    ...
    \begin{tabular}{p{3.8cm}p{8.5cm}}
        ...
        Causality & $y_t$ depends only on current/past $z$'s \\[0.4em]
        Invertibility & $z_t$ recoverable from current/past $y$'s \\
        \bottomrule
    \end{tabular}
    \end{center}

...
\definecolor{mitOrange}{RGB}{227,114,34}
...
% ----------------------------------------------------------------------------
\begin{frame}{VARMA: Causality}
    \begin{definition}
        A VARMA process $\{y_t\}$ is said to be \textbf{causal} with respect to $\{z_t\}$ if
        \[
        y_t \in \Span(z_\tau, \; -\infty < \tau \leq t)
        \]
    \end{definition}
    
    \begin{itemize}
        \item In words: can write $y_t$ as function of \textcolor{mitOrange}{current and lagged white noise realizations}, $z_{t-\ell}$
        
        \vspace{0.5em}
        
        \item Sufficient condition is that \textcolor{mitOrange}{$\Phi(L)$ has 1-sided inverse}, giving VMA($\infty$) representation
        
        \vspace{0.3em}
        
        $\Rightarrow$ Our structural macro models yield VMA representations and so in particular always give causal VARMA processes for the observables $y_t$
    \end{itemize}
\end{frame}

% ----------------------------------------------------------------------------
\begin{frame}{VARMA: Invertibility}
    \begin{definition}
        A VARMA process $\{y_t\}$ is said to be \textbf{invertible} with respect to $\{z_t\}$ if
        \[
        z_t \in \Span(y_\tau, \; -\infty < \tau \leq t)
        \]
    \end{definition}
    
    \begin{itemize}
        \item Can obtain the white noise realizations $z_t$ as a function of \textcolor{mitOrange}{current and lagged values of the process itself}, $y_{t-\ell}$
        
        \vspace{0.5em}
        
        \item Sufficient condition is that \textcolor{mitOrange}{$\Theta(L)$ has 1-sided inverse}, giving a VAR($\infty$) representation
        
        \vspace{0.3em}
        
        $\Rightarrow$ This property is far from guaranteed in our structural models. E.g.\ if we have 5 shocks in $z$ but only 2 observables in $y$, then the process can't possibly be invertible.
    \end{itemize}
\end{frame}
